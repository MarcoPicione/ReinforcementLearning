...
def run(episodes, render = False):
    env = gym.make("weird_space-v0", rows=rows, cols=cols, render_mode="human" if render else None)
    env = TimeLimit(env, max_episode_steps=200)
    q = np.zeros((env.unwrapped.rows, env.unwrapped.cols, env.unwrapped.rows, env.unwrapped.cols, env.action_space.n))
    # print(q.shape)
    ...
    plt.savefig('weird'+str(rows)+'x'+str(cols)+'.png')
    name = "learned_q_.npy"
    np.save(name, q)
...
def main():
    ...
    q = np.load(name)
    print(q)
    env = gym.make("weird_space-v0", rows=rows, cols=cols, render_mode="human")
    state = env.reset()[0]
    terminated = False
